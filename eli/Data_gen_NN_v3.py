...
fn = glob.glob(
    "/global/scratch/users/edkinigstein/Dataset2/F1/MM_Data_set_1_10_18_23_*"
)
    ...
    def __getitem__(self, idx):
        ...
        for i in range(nn_data.shape[1]):
            for j in range(nn_data.shape[2]):
                nn_data[:, i, j] = (
                    nn_data[:, i, j] - np.mean(nn_data[:, i, j])
                ) / np.std(nn_data[:, i, j])
...
for epoch in range(100):  # number of epochs
    ...
    for i in range(
        len(data_gen)
    ):  # here we loop over all of the files in the list for each epoch
        ...
        for q in range(len(nn_data1)):
            split_index = int(
                0.95 * nn_data1[q].shape[0]
            )  # using 95% of data for training and 5% for validation
            train_data1, val_data1 = (
                nn_data1[q][:split_index, :, :, :],
                nn_data1[q][split_index:, :, :, :],
            )
            train_preds1, val_preds1 = (
                predictions1[q][:split_index, :],
                predictions1[q][split_index:, :],
            )
