...
def load_all_datasets(dataset_config: Dict[str, Any], 
                     data_base_folder: Path,
                     transformed: bool = True) -> tuple[dict, dict]:
...
def process_datasets(dataset_config: Dict[str, Any], 
    ...
    dataset_id = generate_dataset_id(c_dataset["seed"], c_dataset["n_samples"], c_dataset["n_shards"])
    ...
    if base_folder_path.exists():
        if overwrite:
            ...
            print(f"Dataset with dataset_id '{dataset_id}' already exists")
            return base_folder_path

    print(f"Creating dataset with dataset_id '{dataset_id}'...")
    ...
    try:
        running_stats_original, running_stats_transformed = create_datasets(
            ...
            c_dataset['n_samples'],
            c_dataset['n_shards'],
            dataset_config.get('splits', [0.7, 0.15, 0.15]),  # default split ratios if not provided
            c_parameters,
    ...
    info_to_print = {
        "original_stats": {key: stats.to_dict(reduced=True) for key, stats in running_stats_original.items()},
        "transformed_stats": {key: stats.to_dict(reduced=True) for key, stats in running_stats_transformed.items()},
    }
    ...
    info_complete = {
        "original_stats": {key: stats.to_dict() for key, stats in running_stats_original.items()},
        "transformed_stats": {key: stats.to_dict() for key, stats in running_stats_transformed.items()},
    }
